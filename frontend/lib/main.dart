...
class ApiService {
  ...
  static Future<Map<String, dynamic>> register(
    ...
    try {
      ...
      return {'success': false, 'message': data['errors']?.toString() ?? 'Erreur'};
    } catch (e) {
      return {'success': false, 'message': 'Impossible de joindre le serveur'};
    }
  }
  ...
  static Future<List<Bourse>> getBourses({String? type, String? pays}) async {
    try {
      ...
      final uri = Uri.parse('$baseUrl/api/bourses/').replace(queryParameters: params);
  ...
  static Future<bool> marquerInteret(String bourseId) async {
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({'user_id': _userId, 'bourse_id': bourseId, 'statut': 'interessé'}),
      );
...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _register() async {
    ...
    final result = await ApiService.register(
      _nomCtrl.text.trim(), _prenomCtrl.text.trim(),
      _emailCtrl.text.trim(), _passCtrl.text,
    );
    setState(() => _loading = false);
    if (result['success']) {
      setState(() { _showRegister = false; _error = null; });
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Compte créé ! Connectez-vous.'), backgroundColor: Color(0xFF2ECC71)),
      );
    } else {
      setState(() => _error = result['message']);
    }
        ...
        child: Center(
          ...
          child: Column(
            ...
            children: [
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(color: const Color(0xFF3B8BEB).withOpacity(0.3)),
                      ),
                      child: const Icon(Icons.school_rounded, color: Color(0xFF3B8BEB), size: 36),
                    ),
                    const SizedBox(height: 16),
                    const Text('SongTaaba', style: TextStyle(color: Colors.white, fontSize: 26, fontWeight: FontWeight.bold)),
                    const Text('Cherchons ensemble', style: TextStyle(color: Color(0xFF6B7080), fontSize: 14)),
                  ],
                ),
              ),
              const SizedBox(height: 48),
              ...
              Row(
                children: [
                  _tabBtn('Connexion', !_showRegister, () => setState(() { _showRegister = false; _error = null; })),
                  const SizedBox(width: 12),
                  _tabBtn('Inscription', _showRegister, () => setState(() { _showRegister = true; _error = null; })),
                ],
              ),
              ...
              _inputField('Email', _emailCtrl, Icons.email_rounded, keyboardType: TextInputType.emailAddress),
              const SizedBox(height: 14),
              _inputField('Mot de passe', _passCtrl, Icons.lock_rounded, obscure: true),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _loading
                      ...
                      : Text(_showRegister ? 'Créer mon compte' : 'Se connecter',
      ...
      child: Column(
        children: [
          Text(label, style: TextStyle(
            ...
            fontSize: 16, fontWeight: active ? FontWeight.bold : FontWeight.normal,
          )),
...
class _HomePageState extends State<HomePage> {
    ...
    return Container(
      ...
      child: BottomNavigationBar(
        ...
        items: const [
          ...
          BottomNavigationBarItem(icon: Icon(Icons.bookmark_rounded), label: 'Favoris'),
  ...
  Widget _buildListeOuEtat() {
    if (_loading) {
      return const Center(child: CircularProgressIndicator(color: Color(0xFF3B8BEB)));
    }
    if (_error != null && _bourses.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.wifi_off_rounded, size: 64, color: Colors.white.withOpacity(0.1)),
            const SizedBox(height: 16),
            Text(_error!, style: TextStyle(color: Colors.white.withOpacity(0.3), fontSize: 15)),
            const SizedBox(height: 16),
            ElevatedButton.icon(
              onPressed: _chargerDonnees,
              icon: const Icon(Icons.refresh_rounded),
              label: const Text('Réessayer'),
              style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF3B8BEB)),
            ),
          ],
        ),
      );
      ...
      child: Row(
        children: [
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(10),
                border: Border.all(color: const Color(0xFF3B8BEB).withOpacity(0.4)),
              ),
              child: const Icon(Icons.person_rounded, color: Color(0xFF3B8BEB), size: 20),
            ),
          ),
            ...
            child: Container(
              ...
              child: TextField(
                ...
                decoration: const InputDecoration(
                  ...
                  prefixIcon: Icon(Icons.search_rounded, color: Color(0xFF6B7080), size: 18),
                  border: InputBorder.none,
          ...
          Container(
            ...
            child: const Icon(Icons.mail_rounded, color: Color(0xFF6B7080), size: 20),
          ),
        ],
      ),
    );
  ...
  Widget _buildFilterChips() {
    final filters = [('tous', 'Toutes'), ('bourse', 'Bourses'), ('stage', 'Stages')];
    return Container(
      height: 44,
          ...
          return Padding(
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: selected ? const Color(0xFF3B8BEB) : const Color(0xFF1A1D26),
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(color: selected ? const Color(0xFF3B8BEB) : const Color(0xFF2A2D3A)),
                ),
  ...
  Widget _buildBourseCard(Bourse bourse) {
    ...
    final accentColor = isBourse ? const Color(0xFF3B8BEB) : const Color(0xFF2ECC71);
                  ...
                  children: [
                    Row(children: [
                      Container(
                        ...
                        child: Text(isBourse ? 'Bourse' : 'Stage',
                          style: TextStyle(color: accentColor, fontSize: 11, fontWeight: FontWeight.w600)),
                      ),
                      const SizedBox(width: 6),
                      Text('🌍 ${bourse.pays}',
                        style: const TextStyle(color: Color(0xFF6B7080), fontSize: 11)),
                    ]),
                    ...
                    Text(bourse.organisme,
                      style: const TextStyle(color: Color(0xFF6B7080), fontSize: 12)),
                  ],
                ),
              ),
            ),
              ...
              child: Column(
                ...
                children: [
                  GestureDetector(
                    ...
                    child: Icon(
                      bourse.isFavori ? Icons.bookmark_rounded : Icons.bookmark_border_rounded,
                      color: bourse.isFavori ? const Color(0xFF3B8BEB) : const Color(0xFF6B7080),
                      size: 20,
                    ),
                  ),
                  if (bourse.deadline != null) ...[
                    ...
                    Container(
                      ...
                      child: Text(bourse.deadline!,
                        style: const TextStyle(color: Color(0xFFE74C3C), fontSize: 10, fontWeight: FontWeight.w500)),
                    ),
                  ],
                ],
              ),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text('Aucune opportunité trouvée',
          ...
          child: favoris.isEmpty
              ? Center(
                  child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Icon(Icons.bookmark_border_rounded, size: 64, color: Colors.white.withOpacity(0.1)),
                    const SizedBox(height: 16),
  ...
  Widget _buildNotifications() {
    final notifications = [
      ('Nouvelle bourse disponible', 'Consultez les dernières opportunités', '10:30'),
              ...
              return Container(
                ...
                child: Row(children: [
                  Container(
                    ...
                    child: const Icon(Icons.notifications_rounded, color: Color(0xFF3B8BEB), size: 20),
                  ),
                  const SizedBox(width: 12),
                  Expanded(child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(n.$1, style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600)),
                      const SizedBox(height: 3),
                      Text(n.$2, style: const TextStyle(color: Color(0xFF6B7080), fontSize: 12)),
                    ],
                  )),
                  Text(n.$3, style: const TextStyle(color: Color(0xFF6B7080), fontSize: 11)),
                ]),
              );
            },
          ),
      ...
      builder: (_) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(bourse.nom, style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 6),
            Text(bourse.organisme, style: const TextStyle(color: Color(0xFF6B7080), fontSize: 14)),
            const SizedBox(height: 20),
            _infoRow(Icons.public_rounded, 'Pays', bourse.pays),
            _infoRow(Icons.school_rounded, 'Niveau', bourse.niveau),
            if (bourse.deadline != null) _infoRow(Icons.calendar_today_rounded, 'Deadline', bourse.deadline!),
            if (bourse.montant != null) _infoRow(Icons.attach_money_rounded, 'Montant', bourse.montant!),
            const SizedBox(height: 24),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text(ok ? 'Intérêt enregistré !' : 'Déjà enregistré ou erreur'),
                ...
                child: const Text('Marquer mon intérêt', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
              ),
            ),
          ],
        ),
  ...
  Widget _infoRow(IconData icon, String label, String value) {
    return Padding(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 10),
        Text('$label : ', style: const TextStyle(color: Color(0xFF6B7080), fontSize: 13)),
      ...
      builder: (_) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            const CircleAvatar(
              radius: 36, backgroundColor: Color(0xFF3B8BEB),
              child: Icon(Icons.person_rounded, size: 36, color: Colors.white),
            ),
            const SizedBox(height: 12),
            Text(ApiService.userName, style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
            Text(ApiService.userEmail, style: const TextStyle(color: Color(0xFF6B7080), fontSize: 13)),
            const SizedBox(height: 24),
            ListTile(
              leading: const Icon(Icons.logout_rounded, color: Color(0xFFE74C3C)),
              title: const Text('Se déconnecter', style: TextStyle(color: Color(0xFFE74C3C))),
              onTap: () {
                ApiService.logout();
